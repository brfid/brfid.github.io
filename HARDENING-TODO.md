...
- Remote `main` contains the hardening contract in `f19f93ab57639cf2f168a33d8888f5e32c703040`, the fully labeled image manifest in `578fa9263feb201688f14c67529b91ed448df2d3`, the five-minute PDP-11 boot safety bound in `9718ebf0fb3ffae009eacfa5a6d80daf045813d7`, and the internal-bridge isolation experiment in `139e7473d73dabaabcad3179b21cb684b34a7582`. Every push used `[nopublish]`; checks pipeline `2800690146` for the latest implementation commit passed quality and full-history secret gates.
...
- No new Pages deployment ran. The public site still uses the last successful standard publication and was not replaced by any failed validation.
- After `139e7473d73dabaabcad3179b21cb684b34a7582`, the 19 focused runner tests, all 214 quality tests, rendered-site verification, shell syntax, image-manifest validation, whitespace checks, and redacted current-tree and full-history secret scans passed. Earlier hardening validation also passed CI lint, dry-run simulation, and a clean hash-locked environment bootstrap.
...
- Commit `139e7473d73dabaabcad3179b21cb684b34a7582` replaced `--network none` with a build-specific bridge created by `docker network create --internal`, retained every other restriction, and removed the bridge during cleanup. Vintage-validation pipeline `2800692115`, job `16176807250`, still timed out after 300 seconds at the identical `cn 1 csr 176500 vector 300 skipped: No CSR` line. Its failure status records five YAML lines, 13 VAX spool lines, and zero PDP-11 bio lines. This rules out `--network none` as the sole cause.
- The retries used expected public bio SHA-256 `a929edcd7fcf3e245f968291438328687dff9ddc1ee74d3f73464ca5083936b5`, derived from successful standard-publication job `16160800424`. No new bio was produced for comparison.
...
2. Do not retry the internal-bridge experiment or increase the boot timeout without new evidence. Pipeline `2800692115` showed that the bridge did not change the deterministic stall.
3. Resume by isolating one remaining runtime change at a time on native amd64 in a non-publishing diagnostic path. Keep the stage-specific mounts and fail-closed image manifest unless evidence identifies them; the failure occurs before the output mount is used.
