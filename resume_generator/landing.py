...
def _build_context(
    *, resume: Resume, out_dir: Path, portfolio_path: str | None = None
) -> LandingContext:
    ...
    Args:
        ...
        portfolio_path: Optional portfolio URL path for landing-page navigation.
    ...
    # Read vintage-generated HTML fragment if available
    contact_html = _read_optional_text(out_dir / "contact.html")
    # Mark as safe HTML to prevent Jinja2 from escaping it
    # Safe because HTML is generated by our own vintage program, not user input
    contact_fragment = Markup(contact_html) if contact_html else None  # noqa: S704

    vintage_log = _read_optional_text(out_dir / "vintage-build.log")
    vintage_log_path = "/vintage-build.log" if vintage_log else None
    ...
    return LandingContext(
        ...
        vintage_log_path=vintage_log_path,
...
def build_landing_page(
    *,
    ...
    """
    ctx = _build_context(resume=resume, out_dir=out_dir, portfolio_path=portfolio_path)
