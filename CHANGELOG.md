...
### Recently Completed
- **2026-03-02:** Codebase simplification: deleted `host_logging` package (ARPANET log collection — not wired into any live path, fully reconstructible) and 12 associated test files; deleted `resume_generator/contact_json.py` (dead code, never imported); removed `docker>=7.0` dependency; removed duplicate quality-check runs on PRs (ci.yml now main-only, test.yml owns PRs+feature branches); aligned pylint scope to `resume_generator tests` in both workflows; added `instance-status-ok` wait to `edcloud_lifecycle.py cmd_start` for parity with deploy.yml.
- **2026-03-02:** Landing page cleanup: single CTA (View Portfolio), distinctive `principal_headline` replacing generic tagline (available via pipeline and `$resume.principal_headline` fallback for fast builds), proof strip removed from home template. `impact_highlights` still generated by pipeline; reserved for resume page.
