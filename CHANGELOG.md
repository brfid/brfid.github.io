...
- Single build mode (vintage). `deploy.yml` triggers on push to `main` (skip with `[nopublish]` in commit message); `workflow_dispatch` available for re-runs.
...
- Landing page hierarchy supports vintage-pipeline-driven `principal_headline` and `impact_highlights` (emitted via `resume.vintage.yaml` -> `brad.bio.txt` -> `hugo/data/bio.yaml`), while `about` remains sourced from `resume.yaml` top-level `about`. `principal_headline` also available via `$resume.principal_headline` fallback for fast/local builds (CI copies root `resume.yaml` to `hugo/data/resume.yaml` before Hugo build).
- Homepage hero: name → `principal_headline` (first-person, from pipeline or `$resume.principal_headline`) → `about` paragraph → single CTA (View Portfolio) → social icons → provenance line (vintage builds only). No proof strip on landing page; `impact_highlights` is generated by pipeline but reserved for the resume page.
...
### Changed
- Single vintage-only build mode: `deploy.yml` trigger migrated from `publish-*` tags to push-to-`main`; fast/local mode and mode detection removed. Every push to `main` deploys (suppress with `[nopublish]`).
...
- Script docstrings updated for Google-style Ruff `D` conformance.
- Lint line-length policy relaxed from 100 to 120 chars (ruff + pylint) to reduce non-semantic churn.
- `edcloud_lifecycle.py cmd_start`: added `instance-status-ok` wait for parity with `deploy.yml`.
- `ci.yml` scoped to main-only; `test.yml` owns PRs and feature branches (removed duplicate quality-check runs).
- pylint scope aligned to `resume_generator tests` in both workflows.

### Removed
- `host_logging` package (ARPANET log collection — not wired into any live path) and 12 associated test files.
- `resume_generator/contact_json.py` (dead code, never imported).
- `docker>=7.0` dependency.
- Legacy `resume_generator` vintage/ARPANET orchestration modules and flags.

### Fixed
- `resume_generator/bio_yaml.py`: narrowed exception handling to `except json.JSONDecodeError` (was broad `except Exception`); fixed CI gate failure on `d7f3c03`.
