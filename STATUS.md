...
- The publish path requires and exposes a curated VAX/PDP-11 build log, links the landing page to the exact enclosing GitHub Actions run, and emits a non-empty `pipeline-status.json` with build and stage metadata.
...
- The Blog publishes `/posts/stracheys-principle/` and `/posts/doc-rot-maintenance-gap/`. Keep the approved resume, public HTML, and phone-free production PDF aligned. The verified pre-rewrite backup at `/Users/brf/src/brfid.github.io-pre-history-rewrite-20260821` must never be pushed.
