...
- GitLab-hosted amd64 runners execute the publish path. Standard mode uses the immutable GitLab Container Registry VAX and PDP-11 image pair pinned in `scripts/vintage-runner.sh`, disables local image fallback, and retains a public fingerprinted, checksummed bio, build log, and status artifact for 90 days.
- The PDP-11 image recipe retrieves its checksum-pinned 2.11BSD disk archive from the project's public GitLab Generic Package Registry. Manual image-build pipelines publish source-commit tags and an immutable digest pair for validation and promotion.
