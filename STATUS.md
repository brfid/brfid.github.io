...
- The working tree builds the landing page, Blog, both RSS feeds, Resume, the `/about/` alias, and `resume.pdf` through Hugo and Playwright. Every HTML page retains the full `noindex` policy; sitemap output remains disabled.
...
- Vintage execution, image promotion, reuse modes, and public vintage provenance are retired in the working tree. `docs/vintage-pipeline.md` links to the original implementation in Git history. Existing hosted artifacts and container packages are unchanged.
- These changes are local and have not been published. The live site still reflects the previous publication.
...
- Publish the pipeline simplification and verify the first hosted build, deployment, and live site.
