...
- GitHub-hosted `ubuntu-latest` runners execute the publish path. Production uses the immutable VAX and PDP-11 image pair pinned in `scripts/vintage-runner.sh`, disables local image fallback, and requires the staged vintage bio, build log, and status.
