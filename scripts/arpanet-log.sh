...
#
# Captures all input with timestamps and writes to host build log path
...
# Create build directory if it doesn't exist (may fail if host path is not accessible from BSD)
mkdir -p "$LOG_DIR" 2>/dev/null || true

# Fallback to /tmp if host path not accessible
if [ ! -d "$LOG_DIR" ]; then
