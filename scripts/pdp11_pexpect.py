#!/usr/bin/env python3
"""Stage A: Render brad.bio.roff via nroff on PDP-11 2.11BSD using pexpect.

Spawns SIMH pdp11 in stdin/stdout mode (no telnet port), boots 2.11BSD,
receives brad.bio.uu (the UUCP spool file produced by the VAX), injects it
via heredoc, uudecodes it to brad.bio.roff, runs nroff (no macro package) to
fill and justify the bio, captures output.

The host acts as a UUCP store-and-forward node: brad.bio.uu arrives from the
VAX spool and is delivered here for decoding and rendering.

Usage (inside Docker container):
    python3 pdp11_pexpect.py --input /build/brad.bio.uu --output /build/brad.bio.txt

Usage (direct, if SIMH + disk image are available locally):
    python3 pdp11_pexpect.py \
        --input build/vintage/brad.bio.uu \
        --output build/vintage/brad.bio.txt \
        --ini vintage/machines/pdp11/configs/pdp11-pexpect.ini \
        --workdir /opt/pdp11

Exit codes:
    0  success — brad.bio.txt written
...
def _deliver_uu_spool(child: pexpect.spawn, uu_text: str, remote_uu_path: str) -> None:
    """Deliver a UUCP spool file (brad.bio.uu) to the PDP-11 guest and decode it.
    ...
    2.11BSD uudecode writes the decoded file (brad.bio.roff, per the spool's
    begin header) into the current directory.
    """
...
def _run_nroff(child: pexpect.spawn) -> str:
    """Run nroff on /tmp/brad.bio.roff and return the rendered text.

    No macro package (-man is deliberately omitted): the bio troff uses only
    base requests (.ll, .po, .nf/.fi, .ad, .sp), so -man would wrongly impose
    man-page structure. Output is captured between unique markers to isolate
    it from terminal echo.
    """
    ...
    child.sendline("nroff -Tlp /tmp/brad.bio.roff < /dev/null > /tmp/brad.bio.txt")
    child.expect(_PROMPT, timeout=_NROFF_TIMEOUT)
    _log("nroff complete")
    nroff_out = child.before or b""

    # Verify the output file was produced.
    child.sendline("ls -l /tmp/brad.bio.txt")
    child.expect(_PROMPT, timeout=_CMD_TIMEOUT)
    ls_out = child.before or b""

    log_console_section("pdp11", "pdp11-nroff", strip_console(nroff_out + b"\n" + ls_out))
    ...
    child.expect("__BRAD_BIO_TXT_BEGIN__", timeout=_CMD_TIMEOUT)
    child.expect("__BRAD_BIO_TXT_END__", timeout=_CMD_TIMEOUT)
...
def _clean_nroff_output(raw: str) -> str:
    ...
    2. Strip leading newline that follows the __BRAD_BIO_TXT_BEGIN__ marker.
...
def main(argv=None) -> int:
    """Run Stage A and write ``brad.bio.txt`` from a VAX-generated UUCP spool.
